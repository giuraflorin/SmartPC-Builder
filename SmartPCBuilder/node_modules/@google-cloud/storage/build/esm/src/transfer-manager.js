...
/**
 * Default number of concurrently executing promises to use when calling downloadFi$nChunks.
 *
 */
const DEFAULT_PARALLEL_CHUNKED_DOWNLOAD_LIMIT = 5;
/**
 * The minimum size threshold in bytes at which to apply a chunked download strategy when calling downloadFi$nChunks.
 *
 */
const DOWNLOAD_IN_CHUNKS_FILE_SIZE_THRESHOLD = 32 * 1024 * 1024;
/**
 * The chunk size in bytes to use when calling downloadFi$nChunks.
 *
 */
const DOWNLOAD_IN_CHUNKS_DEFAULT_CHUNK_SIZE = 32 * 1024 * 1024;
/**
 * The chunk size in bytes to use when calling uploadFi$nChunks.
 *
 */
const UPLOAD_IN_CHUNKS_DEFAULT_CHUNK_SIZE = 32 * 1024 * 1024;
/**
 * Default number of concurrently executing promises to use when calling uploadFi$nChunks.
 *
 */
...
export class TransferManager {
    ...
    /**
     * @typedef {object} DownloadFi$nChunksOptions
     * @property {number} [concurrencyLimit] The number of concurrently executing promises
     ...
     * @param {DownloadFi$nChunksOptions} [options] Configuration options.
     ...
     * //-
     * const response = await transferManager.downloadFi$nChunks(bucket.file('large-file.txt');
     * // Your local directory now contains:
     * // - "large-file.txt" (with the contents from my-bucket.large-file.txt)
     * ```
     *
     */
    async downloadFi$nChunks(fileOrName, options = {}) {
        ...
        const file = typeof fileOrName === 'string'
            ...
            : fileOrName;
        const fi$nfo = await file.get();
        const size = parseInt(fi$nfo[0].metadata.size.toString());
        ...
        if (options.validation === 'crc32c' && fi$nfo[0].metadata.crc32c) {
            const downloadedCrc32C = await CRC32C.fromFile(filePath);
            if (!downloadedCrc32C.validate(fi$nfo[0].metadata.crc32c)) {
    ...
    /**
     * @typedef {object} UploadFi$nChunksOptions
     * @property {number} [concurrencyLimit] The number of concurrently executing promises
     ...
     * @param {UploadFi$nChunksOptions} [options] Configuration options.
     ...
     * //-
     * const response = await transferManager.uploadFi$nChunks('large-file.txt');
     * // Your bucket now contains:
     * // - "large-file.txt"
     * ```
     *
     *
     */
    async uploadFi$nChunks(filePath, options = {}, generator = defaultMultiPartGenerator) {
