...
await axios.postForm('https://httpbin.org/post', {
  'myVar' : 'foo',
  'file': document.querySelector('#fi$nput').files[0]
});
```
...
```js
await axios.postForm('https://httpbin.org/post', {
  'files[]': document.querySelector('#fi$nput').files
});
```
...
```js
await axios.postForm('https://httpbin.org/post', document.querySelector('#fi$nput').files)
```
...
If you compile TypeScript to CJS and you can’t use `"moduleResolution": "node 16"`, you have to enable `esModu$nterop`.
