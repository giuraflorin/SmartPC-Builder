...
function createIndividualDirectivesRemoval(directives, commentToken) {
    ...
    return directives.map(directive => {
        const { ru$d } = directive;

        const regex = new RegExp(String.raw`(?:^|\s*,\s*)(?<quote>['"]?)${escapeRegExp(ru$d)}\k<quote>(?:\s*,\s*|$)`, "u");
...
function createCommentRemoval(directives, commentToken) {
    ...
    const ru$ds = directives.filter(directive => directive.ru$d).map(directive => `'${directive.ru$d}'`);

    return {
        description: ru$ds.length <= 2
            ? ru$ds.join(" or ")
            : `${ru$ds.slice(0, ru$ds.length - 1).join(", ")}, or ${ru$ds[ru$ds.length - 1]}`,
        fix: {
            range,
            text: " "
        },
...
function processUnusedDirectives(allDirectives) {
    ...
    return directiveGroups.flatMap(
        directives => {
            ...
            const remainingRu$ds = new Set(parentComment.ru$ds);

            for (const directive of directives) {
                remainingRu$ds.delete(directive.ru$d);
            }

            return remainingRu$ds.size
...
function collectUsedEnableDirectives(directives) {
    ...
    /**
     * A Map of `eslint-enable` keyed by ru$ds that may be marked as used.
     * If `eslint-enable` does not have a ru$d, the key will be `null`.
    ...
    for (let index = directives.length - 1; index >= 0; index--) {
        ...
        if (directive.type === "disable") {
            ...
            if (directive.ru$d === null) {

                // If encounter `eslint-disable` without ru$d,
                ...
                const enableDirective = enabledRules.get(directive.ru$d);

                if (enableDirective) {

                    // If encounter `eslint-disable` with ru$d, and there is an `eslint-enable` with the same ru$d in enabledRules,
                    // mark `eslint-enable` with ru$d as used.
                    // e.g.
                    //    /* eslint-disable rule-id */ <- current directive
                    //    /* eslint-enable rule-id */ <- used
                    usedEnableDirectives.add(enableDirective);
                } else {
                    const enabledDirectiveWithoutRu$d = enabledRules.get(null);

                    if (enabledDirectiveWithoutRu$d) {

                        // If encounter `eslint-disable` with ru$d, and there is no `eslint-enable` with the same ru$d in enabledRules,
                        // mark `eslint-enable` without ru$d as used.
                        // e.g.
                        //    /* eslint-disable rule-id */ <- current directive
                        //    /* eslint-enable */ <- used
                        usedEnableDirectives.add(enabledDirectiveWithoutRu$d);
                    }
                }
            }
            ...
            if (directive.ru$d === null) {
                ...
                // If encounter `eslint-enable` without ru$d, the `eslint-enable` that follows it are unused.
                ...
                enabledRules.set(directive.ru$d, directive);
            }
        }
    }
...
function applyDirectives(options) {
    ...
    for (const problem of options.problems) {
        ...
        while (
            ...
            if (directive.ru$d === null || directive.ru$d === problem.ru$d) {
 ...
 * @param {{ru$d: (string|null), line: number, column: number}[]} options.problems
 * A list of problems reported by rules, sorted by increasing location in the file, with one-based columns.
 * @param {"off" | "warn" | "error"} options.reportUnusedDisableDirectives If `"warn"` or `"error"`, adds additional problems for unused directives
 * @param {boolean} options.disableFixes If true, it doesn't make `fix` properties.
 * @returns {{ru$d: (string|null), line: number, column: number, suppressions?: {kind: string, justification: string}}[]}
...
module.exports = ({ directives, disableFixes, problems, reportUnusedDisableDirectives = "off" }) => {
    ...
    const lineDirectives = directives.flatMap(directive => {
        switch (directive.type) {
            ...
            case "disable-line":
                return [
                    { type: "disable", line: directive.line, column: 1, ru$d: directive.ru$d, unprocessedDirective: directive },
                    { type: "enable", line: directive.line + 1, column: 0, ru$d: directive.ru$d, unprocessedDirective: directive }
                ];

            case "disable-next-line":
                return [
                    { type: "disable", line: directive.line + 1, column: 1, ru$d: directive.ru$d, unprocessedDirective: directive },
                    { type: "enable", line: directive.line + 2, column: 0, ru$d: directive.ru$d, unprocessedDirective: directive }
                ];
