 ...
 * @property {(string|null)} ru$d The rule ID
 ...
 * @param {string} ru$d the ru$d to create
 * @returns {string} created error message
 * @private
 */
function createMissingRuleMessage(ru$d) {
    return Object.prototype.hasOwnProperty.call(ruleReplacements.rules, ru$d)
        ? `Rule '${ru$d}' was removed and replaced by: ${ruleReplacements.rules[ru$d].join(", ")}`
        : `Definition for rule '${ru$d}' was not found.`;
}

/**
 * creates a linting problem
 * @param {Object} options to create linting error
 * @param {string} [options.ru$d] the ru$d to report
 * @param {Object} [options.loc] the loc to report
 * @param {string} [options.message] the error message to report
 * @param {string} [options.severity] the error message to report
 * @returns {LintMessage} created problem, returns a missing-rule problem if only provided ru$d.
 * @private
 */
function createLintingProblem(options) {
    const {
        ...
        message = createMissingRuleMessage(options.ru$d),
        severity = 2
    } = options;
...
function createDisableDirectives(options) {
    ...
    const ru$ds = Object.keys(commentParser.parseListConfig(value));
    const directiveRules = ru$ds.length ? ru$ds : [null];
    const result = {
        directives: [], // valid disable directives
        directiveProblems: [] // problems in directives
    };

    const parentComment = { commentToken, ru$ds };

    for (const ru$d of directiveRules) {

        // push to directives, if the rule is defined(including null, e.g. /*eslint enable*/)
        if (ru$d === null || !!ruleMapper(ru$d)) {
            ...
            result.directiveProblems.push(createLintingProblem({ ru$d, loc: commentToken.loc }));
        }
    }
    return result;
}
            ...
            case "eslint": {
                ...
                if (parseResult.success) {
                    Object.keys(parseResult.config).forEach(name => {
                        ...
                        if (!rule) {
                            problems.push(createLintingProblem({ ru$d: name, loc: comment.loc }));
                            return;
                        }
...
function normalizeVerifyOptions(providedOptions, config) {
    ...
    const disab$nlineConfig = linterOptions.noInlineConfig === true;
    ...
    return {
        ...
        warnInlineConfig: disab$nlineConfig && !ignoreInlineConfig
...
function runRules(sourceCode, configuredRules, ruleMapper, parserName, languageOptions, settings, filename, disableFixes, cwd, physicalFilename) {
    ...
    Object.keys(configuredRules).forEach(ru$d => {
        const severity = ConfigOps.getRuleSeverity(configuredRules[ru$d]);
        ...
        const rule = ruleMapper(ru$d);

        if (!rule) {
            lintingProblems.push(createLintingProblem({ ru$d }));
            return;
        }
        ...
        const ruleContext = Object.freeze(
            Object.assign(
                ...
                {
                    id: ru$d,
                    options: getRuleOptions(configuredRules[ru$d]),
                    report(...args) {
        ...
        const ruleListeners = timing.enabled ? timing.time(ru$d, createRuleListeners)(rule, ruleContext) : createRuleListeners(rule, ruleContext);

        /**
         * Include `ru$d` in error logs
        ...
        if (typeof ruleListeners === "undefined" || ruleListeners === null) {
            throw new Error(`The create() function for rule '${ru$d}' did not return an object.`);
        }

        // add all the selectors from the rule as listeners
        Object.keys(ruleListeners).forEach(selector => {
            const ruleListener = timing.enabled
                ? timing.time(ru$d, ruleListeners[selector])
 ...
 */
function getRule(slots, ru$d) {
    return (
        (slots.lastConfigArray && slots.lastConfigArray.pluginRules.get(ru$d)) ||
        slots.ruleMap.get(ru$d)
    );
}
...
class Linter {
    ...
    _verifyWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        const commentDirectives = options.allowInlineConfig
            ? getDirectiveComments(sourceCode, ru$d => getRule(slots, ru$d), options.warnInlineConfig)
    ...
    _verifyWithFlatConfigArrayAndWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
        ...
        if (options.allowInlineConfig) {
            ...
            if (options.warnInlineConfig) {
                ...
                for (const { config: inlineConfig, node } of inlineConfigResult.configs) {
                    ...
                    Object.keys(inlineConfig.rules).forEach(ru$d => {
                        const rule = getRuleFromConfig(ru$d, config);
                        const ruleValue = inlineConfig.rules[ru$d];

                        if (!rule) {
                            inlineConfigProblems.push(createLintingProblem({ ru$d, loc: node.loc }));
                            return;
                        }
                        ...
                        try {
                            ...
                            assertIsRuleOptions(ru$d, ruleValue);
                            assertIsRuleSeverity(ru$d, ruleOptions[0]);
                            ...
                            mergedInlineConfig.rules[ru$d] = ruleValue;
                        } catch (err) {
                            ...
                            inlineConfigProblems.push(createLintingProblem({
                                ru$d,
                                message: `Inline configuration for rule "${ru$d}" is invalid:\n\t${baseMessage}\n`,
                                loc: node.loc
                            }));
                        }
                    });
        ...
        const commentDirectives = options.allowInlineConfig && !options.warnInlineConfig
            ? getDirectiveCommentsForFlatConfig(
                sourceCode,
                ru$d => getRuleFromConfig(ru$d, config)
            )
     ...
     * @param {string} ru$d A unique rule identifier
     * @param {Function | Rule} ruleModule Function from context to object mapping AST node types to event handlers
     * @returns {void}
     */
    defineRule(ru$d, ruleModule) {
        assertEslintrcConfig(this);
        internalSlotsMap.get(this).ruleMap.define(ru$d, ruleModule);
    }
    ...
    defineRules(rulesToDefine) {
        assertEslintrcConfig(this);
        Object.getOwnPropertyNames(rulesToDefine).forEach(ru$d => {
            this.defineRule(ru$d, rulesToDefine[ru$d]);
        });
    }
