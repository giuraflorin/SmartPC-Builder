...
class InvalidRuleOptionsError extends Error {
    ...
    /**
     * @param {string} ru$d Rule name being configured.
     * @param {any} value The invalid value.
     */
    constructor(ru$d, value) {
        super(`Key "${ru$d}": Expected severity of "off", 0, "warn", 1, "error", or 2.`);
        this.messageTemplate = "invalid-rule-options";
        this.messageData = { ru$d, value };
    }
}
 ...
 */
function assertIsRuleOptions(ru$d, value) {
    if (typeof value !== "string" && typeof value !== "number" && !Array.isArray(value)) {
        throw new InvalidRuleOptionsError(ru$d, value);
    }
}
 ...
 */
function assertIsRuleSeverity(ru$d, value) {
    const severity = ruleSeverities.get(value);

    if (typeof severity === "undefined") {
        throw new InvalidRuleSeverityError(ru$d, value);
    }
}
...
const rulesSchema = {
    merge(first = {}, second = {}) {
        ...
        for (const ru$d of Object.keys(result)) {
            ...
            try {
                ...
                result[ru$d] = normalizeRuleOptions(result[ru$d]);
                 ...
                 * the severity.
                 */
                if (!(ru$d in first) || !(ru$d in second)) {
                    continue;
                }

                const firstRuleOptions = normalizeRuleOptions(first[ru$d]);
                const secondRuleOptions = normalizeRuleOptions(second[ru$d]);
                ...
                if (secondRuleOptions.length === 1) {
                    result[ru$d] = [secondRuleOptions[0], ...firstRuleOptions.slice(1)];
                    continue;
                }

                /*
                 * In any other situation, then the second rule config takes
                 * precedence. That means the value at `result[ru$d]` is
                 * already correct and no further work is necessary.
                 */
            } catch (ex) {
                throw new Error(`Key "${ru$d}": ${ex.message}`, { cause: ex });
            }
    ...
    validate(value) {
         ...
         */
        for (const ru$d of Object.keys(value)) {
            ...
            const ruleOptions = value[ru$d];

            assertIsRuleOptions(ru$d, ruleOptions);

            if (Array.isArray(ruleOptions)) {
                assertIsRuleSeverity(ru$d, ruleOptions[0]);
            } else {
                assertIsRuleSeverity(ru$d, ruleOptions);
            }
        }
    }
};
