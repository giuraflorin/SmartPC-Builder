...
/** @typedef {import("../shared/types").DeprecatedRu$nfo} DeprecatedRu$nfo */
...
/** @type {WeakMap<ExtractedConfig, DeprecatedRu$nfo[]>} */
 ...
 * @returns {DeprecatedRu$nfo[]} The used deprecated rule list.
 */
function getOrFindUsedDeprecatedRules(cliEngine, maybeFilePath) {
    ...
    if (!usedDeprecatedRulesCache.has(config)) {
        ...
        for (const [ru$d, ruleConf] of Object.entries(config.rules)) {
            if (getRuleSeverity(ruleConf) === 0) {
                continue;
            }
            const rule = pluginRules.get(ru$d) || BuiltinRules.get(ru$d);
            const meta = rule && rule.meta;

            if (meta && meta.deprecated) {
                retv.push({ ru$d, replacedBy: meta.replacedBy || [] });
            }
        }
...
class ESLint {
     ...
     * @returns {Object} A mapping of ru$ds to rule meta objects.
     */
    getRulesMetaForResults(results) {

        const resultRu$ds = new Set();

        // first gather all ru$ds from all results

        for (const result of results) {
            for (const { ru$d } of result.messages) {
                resultRu$ds.add(ru$d);
            }
            for (const { ru$d } of result.suppressedMessages) {
                resultRu$ds.add(ru$d);
            }
        }
        ...
        for (const [ru$d, rule] of rules) {
            if (resultRu$ds.has(ru$d)) {
                resultRules.set(ru$d, rule);
            }
        }
