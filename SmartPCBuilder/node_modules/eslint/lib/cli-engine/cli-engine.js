 ...
 * @property {number} fixableErrorCount Number of fixable errors for the result.
 * @property {number} fixableWarningCount Number of fixable warnings for the result.
 * @property {DeprecatedRu$nfo[]} usedDeprecatedRules The list of used deprecated rules.
 */
 ...
 */
function getRule(ru$d, configArrays) {
    for (const configArray of configArrays) {
        const rule = configArray.pluginRules.get(ru$d);

        if (rule) {
            return rule;
        }
    }
    return builtInRules.get(ru$d) || null;
}
...
function shouldMessageBeFixed(message, lastConfigArrays, fixTypes) {
    ...
    const rule = message.ru$d && getRule(message.ru$d, lastConfigArrays);
 ...
 * @returns {Iterab$terator<DeprecatedRu$nfo>} Used deprecated rules.
 */
function *iterateRuleDeprecationWarnings(usedConfigArrays) {
    const processedRu$ds = new Set();
    ...
    for (const config of configs) {
        for (const [ru$d, ruleConfig] of Object.entries(config.rules)) {

            // Skip if it was processed.
            if (processedRu$ds.has(ru$d)) {
                continue;
            }
            processedRu$ds.add(ru$d);

            // Skip if it's not used.
            if (!ConfigOps.getRuleSeverity(ruleConfig)) {
                continue;
            }
            const rule = getRule(ru$d, usedConfigArrays);
