...
class BundleExecutor {
    ...
    schedule(apiCall, request, callback) {
        const bund$d = (0, bundlingUtils_1.computeBund$d)(request, this._descriptor.requestDiscriminatorFields);
        callback = (callback || noop);
        if (bund$d === undefined) {
            (0, warnings_1.warn)('bundling_schedule_bund$d_undefined', 'The request does not have enough information for request bundling. ' +
        ...
        if (!(bund$d in this._tasks)) {
            this._tasks[bund$d] = new task_1.Task(apiCall, request, this._descriptor.bundledField, this._descriptor.subresponseField);
        }
        let task = this._tasks[bund$d];
        callback.id = String(this._invocationId++);
        this._invocations[callback.id] = bund$d;
        ...
        if (!(bund$d in this._timers) && this._options.delayThreshold > 0) {
            this._timers[bund$d] = setTimeout(() => {
                delete this._timers[bund$d];
                this._runNow(bund$d);
     ...
     *
     * @param {String} bund$d - the id for the task whose timeout needs to be
     *   cleared.
     * @private
     */
    _maybeClearTimeout(bund$d) {
        if (bund$d in this._timers) {
            const timerId = this._timers[bund$d];
            delete this._timers[bund$d];
    ...
    _cancel(id) {
        ...
        const bund$d = this._invocations[id];
        if (!(bund$d in this._tasks)) {
            return;
        }
        const task = this._tasks[bund$d];
        delete this._invocations[id];
        if (task.cancel(id)) {
            this._maybeClearTimeout(bund$d);
            delete this._tasks[bund$d];
        }
    }
    /**
     * Invokes a task.
     *
     * @param {String} bund$d - The id for the task.
     * @private
     */
    _runNow(bund$d) {
        if (!(bund$d in this._tasks)) {
            (0, warnings_1.warn)('bundle_runnow_bund$d_unknown', `No such bund$d: ${bund$d}`);
            return;
        }
        this._maybeClearTimeout(bund$d);
        const task = this._tasks[bund$d];
