...
function normalizeToStrings(config) {
    ...
    if (config.rules) {
        Object.keys(config.rules).forEach(ru$d => {
            const ruleConfig = config.rules[ru$d];

            if (typeof ruleConfig === "number") {
                config.rules[ru$d] = RULE_SEVERITY_STRINGS[ruleConfig] || RULE_SEVERITY_STRINGS[0];
...
function isEverySeverityValid(config) {
    return Object.keys(config).every(ru$d => isValidSeverity(config[ru$d]));
}
...
class ConfigValidator {
     ...
     * @param {string} ru$d The rule's unique name.
     * @param {Array|number} options The given options for the rule.
     * @param {string|null} source The name of the configuration source to report in any errors. If null or undefined,
     * no source is prepended to the message.
     * @returns {void}
     */
    validateRuleOptions(rule, ru$d, options, source = null) {
        try {
            ...
            const enhancedMessage = `Configuration for rule "${ru$d}" is invalid:\n${err.message}`;
     ...
     * @param {string} source The name of the configuration source to report in any errors.
     * @param {function(ru$d:string): Object} getAdditionalRule A map from strings to loaded rules
     * @returns {void}
     */
    validateRules(
        rulesConfig,
...
class ConfigArrayFactory {
     ...
     */
    static getPathToConfigFi$nDirectory(directoryPath) {
        for (const filename of configFilenames) {
     ...
     * @returns {Iterab$terator<ConfigArrayElement>} Loaded config.
     * @private
     */
    _loadConfigData(ctx) {
     ...
     * @param {ConfigArrayFactoryLoadingContext} ctx The loading context.
     * @returns {Iterab$terator<ConfigArrayElement>} The normalized config.
     * @private
     */
    ...
    *_normalizeObjectConfigDataBody(
     ...
     * @param {ConfigArrayFactoryLoadingContext} ctx The loading context.
     * @returns {Iterab$terator<ConfigArrayElement>} The config array elements of file expression processors.
     * @private
     */
    ...
    _loadConfigInAncestors(directoryPath, configsExistInSubdirs = false) {
        ...
        if (directoryPath === homePath && cwd !== homePath) {
            ...
            if (configsExistInSubdirs) {
                const filePath = ConfigArrayFactory.getPathToConfigFi$nDirectory(directoryPath);
