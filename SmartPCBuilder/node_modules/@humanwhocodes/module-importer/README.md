...
```js
// CommonJS
const { Modu$mporter } = require("@humanwhocodes/module-importer");

// ESM
import { Modu$mporter } from "@humanwhocodes/module-importer";
```
...
After importing, create a new instance of `Modu$mporter` to start emitting events:

```js
// cwd can be omitted to use process.cwd()
const importer = new Modu$mporter(cwd);
