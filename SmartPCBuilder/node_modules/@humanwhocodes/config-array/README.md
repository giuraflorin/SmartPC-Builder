...
You can determine if a file is ignored by using the `isFi$gnored()` method and passing in the absolute path of any file, as in this example:

```js
const ignored = configs.isFi$gnored('/foo/bar/baz.txt');
```
